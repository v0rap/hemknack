...
var _WebWindow_instances, _WebWindow_handleResize, _WebWindow_handleMouseDown, _WebWindowManager_instances, _WebWindowManager_handleMouseOver, _WebWindowManager_handleMouseUp;
Object.defineProperty(exports, "__esModule", { value: true });
exports.WebWindowEvent = void 0;
class WebWindow {
    constructor(title, x, y, width, height, windowManager, hash) {
        _WebWindow_instances.add(this);
        this.startX = 0;
        this.startY = 0;
        this.startClientX = 0;
        this.startClientY = 0;
        ...
        titleBar.addEventListener('mousedown', event => __classPrivateFieldGet(this, _WebWindow_instances, "m", _WebWindow_handleMouseDown).call(this, event));
        ...
        resizeAreaRight.addEventListener('mousedown', event => __classPrivateFieldGet(this, _WebWindow_instances, "m", _WebWindow_handleResize).call(this, event));
        ...
        resizeAreaLeft.addEventListener('mousedown', event => __classPrivateFieldGet(this, _WebWindow_instances, "m", _WebWindow_handleResize).call(this, event));
        ...
        resizeAreaBottom.addEventListener('mousedown', event => __classPrivateFieldGet(this, _WebWindow_instances, "m", _WebWindow_handleResize).call(this, event));
        ...
        resizeAreaBL.addEventListener('mousedown', event => __classPrivateFieldGet(this, _WebWindow_instances, "m", _WebWindow_handleResize).call(this, event));
        ...
        resizeAreaBR.addEventListener('mousedown', event => __classPrivateFieldGet(this, _WebWindow_instances, "m", _WebWindow_handleResize).call(this, event));
    ...
    close() {
        ...
        if (this.onclose !== undefined)
            this.onclose(new WebWindowEvent(this, 'close'));
    }
...
_WebWindow_instances = new WeakSet(), _WebWindow_handleResize = function _WebWindow_handleResize(event) {
    ...
    if (this.startClientY - this.startY >= this.height - this.resizeHandleSize / 2)
        this.windowManager.resize[0] = 'bottom';
    if (this.startClientX - this.startX >= this.width - this.resizeHandleSize / 2)
        this.windowManager.resize[1] = 'right';
    if (this.startClientX - this.startX <= this.resizeHandleSize / 2)
        this.windowManager.resize[1] = 'left';
    this.domElement.dataset.resize = 'true';
}, _WebWindow_handleMouseDown = function _WebWindow_handleMouseDown(event) {
...
class WebWindowManager {
    constructor() {
        _WebWindowManager_instances.add(this);
        ...
        document.addEventListener('mousemove', event => __classPrivateFieldGet(this, _WebWindowManager_instances, "m", _WebWindowManager_handleMouseOver).call(this, event));
        document.addEventListener('mouseup', event => __classPrivateFieldGet(this, _WebWindowManager_instances, "m", _WebWindowManager_handleMouseUp).call(this, event));
    ...
    setActive(_window) {
        if (this.activeWindow !== undefined && _window.hash === this.activeWindow.hash)
            return;
        this.windows.forEach(element => {
            if (element.index > _window.index)
                element.setIndex(element.index - 1);
        });
        if (this.activeWindow !== undefined)
            this.activeWindow.domElement.dataset.active = 'false';
        this.activeWindow = _window;
        this.activeWindow.domElement.dataset.active = 'true';
        this.activeWindow.setIndex(this.windows.size - 1);
    }
    setFocus(_window, focus) {
        this.blocker.dataset.block = focus.toString();
        this.setActive(_window);
    }
    addWindow(title, width = 500, height = 300, x = null, y = null) {
        if (x === null || y === null) {
            if (this.lastY + height + this.borderOffset > window.innerHeight)
                this.lastY = this.borderOffset;
            if (this.lastX + width + this.borderOffset > window.innerWidth)
                this.lastX = this.borderOffset;
            x = this.lastX;
            y = this.lastY;
...
_WebWindowManager_instances = new WeakSet(), _WebWindowManager_handleMouseOver = function _WebWindowManager_handleMouseOver(event) {
    if (this.activeWindow === undefined)
        return;
    if (this.holding) {
        event.preventDefault();
        return this.activeWindow.updatePos(event.clientX - this.activeWindow.startClientX + this.activeWindow.startX, event.clientY - this.activeWindow.startClientY + this.activeWindow.startY);
    }
    if (this.resize.length > 0) {
        event.preventDefault();
        return this.activeWindow.resize(event.clientX, event.clientY);
    }
}, _WebWindowManager_handleMouseUp = function _WebWindowManager_handleMouseUp(event) {
    this.holding = false;
    if (this.activeWindow === undefined)
        return;
...
class WebWindowEvent {
    constructor(_window, eventType) {
        this.target = _window;
        this.eventType = eventType;
    }
}
exports.WebWindowEvent = WebWindowEvent;
exports.default = WebWindowManager;
